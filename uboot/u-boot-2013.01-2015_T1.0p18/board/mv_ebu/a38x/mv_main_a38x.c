...
#endif
#include "commonheaders/microsemimacros.h"
#include "../common/galileo/gpio.h"
...
#endif
MICROSEMI_BEGIN( )
extern void  getBootRevisionNumber(char* uBootRevNumber);
MICROSEMI_END( )
...
MICROSEMI_BEGIN( )
extern void buildBootCmdArg(char* bootArgRoot, char* bootCmdArg );
extern void swapFlagsInBothROM(void);  
MICROSEMI_END( )
...
MICROSEMI_BEGIN( )
	...
	mvTwsiInit(1, CONFIG_SYS_I2C_SPEED, CONFIG_SYS_TCLK, &slave, 0);
MICROSEMI_END( )
#endif
...
MICROSEMI_BEGIN( )
    char bootArgRoot[256] = {0};
    char bootCmdArg[512]  = {0};
    char uBootRev[64] = "";
    MV_BOOL isRunnigDebugBuild = 0;
    MV_BOOL isManufactureMode = isManufacturingMode();
    #if defined(BOARD_RUN_IN_DEBUG_MODE)
        isRunnigDebugBuild = 1;
    #endif 
    if (isManufactureMode || isRunnigDebugBuild) 
    {
       setMPP0ToUART();
    }    
    getBootRevisionNumber(uBootRev);
    printf("\n\n%s\n", version_string);
    printf("U-Boot Revision: %s\n", uBootRev);
    swapFlagsInBothROM( );
    buildBootCmdArg(bootArgRoot, bootCmdArg);
MICROSEMI_END( )
...
MICROSEMI_BEGIN( )
    /* Galileo requests u-boot loads OS from eMMC by befault */ 
	/* linux boot arguments */
	env = getenv("bootargs_root");
    if((env) && strstr(env, "/dev/nfs")) 
	{
        // keep it for network booting
	}
	else if (0 != strcmp(env, (const char*)bootArgRoot))
    {
	   setenv("bootargs_root", (const char*)bootArgRoot);
    }
	else
    {
        // place holder	
...
MICROSEMI_BEGIN( )
	/* Flatten Device Tree environment setup */
		...
		/* Galileo: Temporary Fix for fdt_skip_update=no */
		fdt_env_setup("/boot/armada-38x-modular-linux-3.10.70-2015_T1.1p9.dtb", MV_TRUE); /* static setup: Skip DT update for customer */
	#else
...
MICROSEMI_BEGIN( )
    /* Galileo requests u-boot loads OS from eMMC by befault */
    if((env) && strstr(env, "nfsroot"))
    {
        // keep the bootcmd argument to allow it boot from network
	}
	else if (0 != strcmp(env, (const char*)bootCmdArg))
    {
	   setenv("bootcmd", (const char*)bootCmdArg);
    }
	else
	{
        //a place holder
