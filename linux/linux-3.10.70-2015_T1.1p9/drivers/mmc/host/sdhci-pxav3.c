...
static int sdhci_pxav3_probe(struct platform_device *pdev)
{
	...
	if (match) {
		...
		if (of_device_is_compatible(np, "marvell,armada-380-sdhci")) {
			...
			/*
			 * Set SD source clock to 100MHz and set it as ref clock
			 * for the MMC stack.
			 */
			clk = devm_clk_get(dev, "corediv");
			if (!IS_ERR(clk)) {
				pxa->gate_clk = pltfm_host->clk;
				pltfm_host->clk = clk;
				clk_prepare_enable(clk);
				clk_set_rate(clk, 100000000);
				dev_info(dev, "registered corediv clock @%ld MHz\n",
					 clk_get_rate(clk) / 1000 / 1000);
			}
		}
		/* Disable DDR50 unconditionally */
...
err_add_host:
	if (pxa->gate_clk) {
		clk_disable_unprepare(pxa->gate_clk);
	}
